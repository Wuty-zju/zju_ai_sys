...
hidden_channels = 8  # You can adjust this hyperparameter
...
for epoch in range(1, epochs+1):
    ...
    if valid_auc > best_valid_auc:
        ...
        torch.save(model, os.path.join(save_dir, model_filename))
    if epoch % 10 == 0:
...
model = torch.load(os.path.join(save_dir, model_filename))
model = model.to(device)  # 确保模型加载到指定设备
